...
def smiles_generator(smiles):
    ...
    generated_selfies = generate_compounds(selfie)
...
def main(smiles):
    ...
    generated_smiles, generated_selfies = smiles_generator(smiles)
...
def predict_page():
    ...
    return send_file(io.BytesIO(file.getvalue().encode()), as_attachment=True, attachment_filename='GeneratedSmiles.csv')
    # return send_file(file, as_attachment=True, attachment_filename='data.csv')
    
    # Dowload dataset

    # Display the predicted compound characteristics on the web page
    # return render_template('pandas.html',
    #                        PageTitle = "Pandas",
    #                        table=[dataset.to_html(classes='data', index = False)], titles= dataset.columns.values)
